// =============================================================================
// AdminPlans.js - Subscription Plans Management Page
// =============================================================================
// This component allows admin users to create, view, edit, and delete subscription plans.
// It provides a modal-based interface for managing plan details.
// =============================================================================
...
function AdminPlans({ navigateTo, handleSignOut }) {
  ...
  // ---------------------------------------------------------------------------
  const { session, user, isAdmin, isLoading: authLoading } = useAuth();
  ...
  // Open modal for viewing plan details (read-only)
  ...
  // Create or update a plan based on modal mode
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare plan data with converted numeric fields
      const planData = {
        ...
        max_session_duration_hours: formData.max_session_duration_hours 
          ? parseFloat(formData.max_session_duration_hours) 
          : null,
        fast_charging_access: formData.fast_charging_access,
        priority_access: formData.priority_access,
        cooldown_percentage: formData.cooldown_percentage 
          ? parseFloat(formData.cooldown_percentage) 
          : null,
        cooldown_time_hour: formData.cooldown_time_hour 
          ? parseFloat(formData.cooldown_time_hour) 
          : null,
      ...
      >
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div 
            ...
            style={{ background: 'radial-gradient(circle, rgba(249, 210, 23, 0.2) 0%, rgba(249, 210, 23, 0.05) 50%, transparent 100%)' }}
          />
          <div 
            className="absolute -bottom-40 -left-40 w-96 h-96 rounded-full blur-3xl animate-float-slow-delay" 
            style={{ background: 'radial-gradient(circle, rgba(56, 182, 255, 0.2) 0%, rgba(56, 182, 255, 0.05) 50%, transparent 100%)' }}
          />
        </div>
        <div 
        ...
        >
          <div 
            ...
            style={{ borderColor: '#38b6ff', borderTopColor: 'transparent' }}
          />
  ...
  if (!isAdmin) {
    return (
      ...
      >
        ...
        <div 
          className="relative z-10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/30 text-center" 
          style={{
            background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.4) 0%, rgba(255, 255, 255, 0.2) 100%)',
            boxShadow: '0 8px 32px 0 rgba(0, 11, 61, 0.15), inset 0 1px 0 0 rgba(255, 255, 255, 0.5)'
          }}
  ...
  return (
    <div 
      className="min-h-screen flex flex-col items-center p-4 text-gray-800 relative overflow-hidden" 
      style={{ background: 'linear-gradient(135deg, #f1f3e0 0%, #e8eae0 50%, #f1f3e0 100%)' }}
    >
      {/* ---------------------------------------------------------------------------
      Animated Background
      ...
      --------------------------------------------------------------------------- */}
      <div 
        className="w-full max-w-7xl mx-auto pt-24 pb-8 relative z-10 px-4 sm:px-6 lg:px-8" 
        style={{ 
      ...
      >
        ...
        ----------------------------------------------------------------------- */}
        <div 
          className="relative backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/30 overflow-hidden py-8 px-8 mb-8" 
          style={{ 
            background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.4) 0%, rgba(255, 255, 255, 0.2) 100%)',
            boxShadow: '0 8px 32px 0 rgba(0, 11, 61, 0.15), inset 0 1px 0 0 rgba(255, 255, 255, 0.5)'
          }}
        >
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-4xl sm:text-5xl font-bold mb-2" style={{ color: '#000b3d' }}>
                Subscription Plans Management
              </h1>
              <p className="text-lg" style={{ color: '#000b3d', opacity: 0.7 }}>
        ...
        {error && (
          <div 
            className="relative backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 overflow-hidden py-4 px-6 mb-6" 
            style={{ 
        ...
        ----------------------------------------------------------------------- */}
        <div 
          className="relative backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 overflow-hidden" 
          style={{ 
            background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.4) 0%, rgba(255, 255, 255, 0.2) 100%)',
            boxShadow: '0 8px 32px 0 rgba(0, 11, 61, 0.15), inset 0 1px 0 0 rgba(255, 255, 255, 0.5)'
          }}
        >
          <div 
            className="px-6 py-4" 
            style={{ background: 'rgba(255, 255, 255, 0.1)', borderBottom: '1px solid rgba(255, 255, 255, 0.3)' }}
          >
          ...
          {loading ? (
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ background: 'rgba(255, 255, 255, 0.1)', borderBottom: '1px solid rgba(255, 255, 255, 0.3)' }}>
                    {['Plan Name', 'Price', 'Duration', 'Daily Limit', 'PayPal Link', 'Features', 'Created', 'Actions'].map(header => (
                      <th 
                        key={header} 
                  ...
                  {plans.map((plan, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div 
                            className={`text-sm font-medium ${plan.plan_name.includes('(DISCONTINUED)') ? 'line-through' : ''}`} 
                            style={{
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {plan.max_session_duration_hours && (
                          <div className="text-sm" style={{ color: '#000b3d', opacity: 0.7 }}>
                            {plan.max_session_duration_hours}h max session
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          ...
                          <button
                            ...
                            className="font-semibold transition-colors duration-200"
                            style={{ 
                              color: plan.plan_name.includes('(DISCONTINUED)') ? '#f9d217' : '#ef4444' 
                            }}
            ...
            <div className="p-8 text-center">
              <div className="text-lg mb-4" style={{ color: '#000b3d', opacity: 0.6 }}>
      ...
      {showModal && (
        <div 
          className="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50 backdrop-blur-sm flex items-start justify-center pt-20 pb-20"
        >
          <div 
            className="relative backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 overflow-hidden p-6 w-full max-w-2xl" 
            style={{ 
              background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.4) 0%, rgba(255, 255, 255, 0.2) 100%)',
              boxShadow: '0 8px 32px 0 rgba(0, 11, 61, 0.15), inset 0 1px 0 0 rgba(255, 255, 255, 0.5)'
            }}
          >
            ...
            {modalMode === 'view' ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    >
                      <label className="block text-sm font-bold mb-1" style={{ color: '#000b3d', opacity: 0.8 }}>
                        {item.label}
                      </label>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Daily Limit and Max Session Duration Row */}
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                {/* Cooldown Percentage and Time Row */}
                <div className="grid grid-cols-2 gap-4">
                  <div>

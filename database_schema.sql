...
CREATE TABLE public.quota_extension_pricing (
  ...
  paypal_link text,
  extension_amount_mah integer DEFAULT 1000,
  price_per_transaction numeric DEFAULT 10.00,
...
CREATE TABLE public.quota_extensions (
  ...
  CONSTRAINT quota_extensions_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(user_id)
);
CREATE TABLE public.quota_pricing_history (
  ...
  CONSTRAINT quota_pricing_history_admin_user_id_fkey FOREIGN KEY (admin_user_id) REFERENCES public.users(user_id)
);

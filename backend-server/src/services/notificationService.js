...
function getUserNotifications(userId, limit = 50, offset = 0) {
  return pool.query(`
    ...
    LIMIT $2 OFFSET $3
  `, [userId, parseInt(limit), parseInt(offset)]).then(res => res.rows);
}
...
function getUnreadCount(userId) {
  return pool.query(
    ...
    [userId]
  ).then(res => parseInt(res.rows[0].unread_count));
}
...
async function markAllRead(userId) {
  ...
  return { message: 'All notifications marked as read', updatedCount: result.rows.length };
}

Okay, I will update your `server.js` file to implement the **single ESP32 controlling multiple ports** logic.

This updated code will:

  * Use `ESP32_CHARGER_STATION_001` as the `deviceId` for the main station/ESP32.
  * Expect `port_number` in MQTT payloads from the ESP32.
  * Correctly look up `charging_port` entries using `device_mqtt_id` (the station ID) AND `port_number_in_device`.
  * Properly manage `charging_session` creation/resumption and link `consumption_data` inserts with a valid `session_id`.
  * Update `device_status_logs`, `current_device_status`, and `charging_port` records correctly for each port.
  * Update the API endpoints to accept `portNumber` in the URL for consumption and control.

**Please review this code carefully.** It's a significant merge of concepts.
...
mqttClient.on('message', async (topic, message) => {
    ...
    try {
        ...
        // Skip processing if port_number is invalid or missing for a usage/status message
        // UNLESS it's the specific station-level 'online' or 'offline' status without port details.
        if ((topic.startsWith('charger/usage/') || topic.startsWith('charger/status/')) && (portNumberInDevice === undefined || portNumberInDevice < 1)) {
            if (topic === `charger/status/${ESP32_STATION_CLIENT_ID}` && (payload.status === 'online' || payload.status === 'offline')) {
                // This is the overall station status. You can log it or update a dedicated station_status table.
                console.log(`Station ${deviceId} is ${payload.status}. No specific port_id for this message.`);
                // Return as it doesn't map to a specific port_id in DB with this schema.
                return;
            }
        ...
        // --- Core Logic: Find the actual port_id (UUID) from charging_port table ---
        ...
        if (!actualPortId) {
            ...
            return; // Cannot process if a specific port mapping is not found
        }
        ...
        if (topic.startsWith('charger/usage/')) {
            ...
            if (charger_state === 'ON') {
                ...
                if (currentSessionId) { // This check is crucial to prevent null session_id inserts
                    ...
                    console.error(`ERROR: Charger ON for ${deviceId} Port ${portNumberInDevice} but currentSessionId is still null after session creation attempt. Skipping consumption data insert.`);
                }
                ...
                // IMPORTANT: Consumption data is NOT inserted for OFF states.
            }
        }
...
// Make sure these match what you have if you removed them
// Example:
/*
...
// ... other station/reading/user CRUD routes ...
*/
...
app.post('/api/esp32/command', async (req, res) => {
    ...
    let message = '';
    // This part assumes a very specific legacy message format, might need updating

...
mqttClient.on('message', async (topic, message) => {
    ...
    try {
        ...
        if (topic.startsWith('charger/usage/')) {
            ...
            const sessionKey = `${deviceId}_${port_number}`;
            let currentSessionId = activeChargerSessions[sessionKey];

            // --- Step 3: Handle ON state (Start/Resume Session) ---
            // This entire 'if' block ONLY executes if charger_state is 'ON'
            if (charger_state === 'ON') {
                if (!currentSessionId) {
                    // Check existing active session in DB (for backend restarts)
                    ...
                    if (existingActiveSession.rows.length === 0) {
                        ...
                        activeChargerSessions[sessionKey] = currentSessionId;
                        console.log(`Resumed existing active session ${currentSessionId} for port ${actualPortId} (Device: ${deviceId}, PortNum: ${port_number})`);
                    }
                }
                ...
                if (currentSessionId) { // This condition MUST be true for the insert to proceed
                    ...
                    console.log(`Stored consumption for ${deviceId} Port ${port_number} in session ${currentSessionId}: ${consumption}W`);
        
                    // --- Step 5: Update energy_consumed_kwh in charging_session (also only if ON) ---
